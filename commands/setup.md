...
If 1 or 2: copy the relevant files from `${CLAUDE_PLUGIN_ROOT}/examples/` into `{experts_clones_path}/examples/`. Inform the user of the path and suggest they use it as a reference when creating new clones.
...
Write `${CLAUDE_PLUGIN_ROOT}/config.yml` with this structure:
